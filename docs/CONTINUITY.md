...
- **当前阶段**：Phase 1（Pull 路径最小可运行 Demo）已完成并可运行。
- **一键验证**：`COMPOSE_BAKE=true ./scripts/demo.sh table-stats --with-health`

---

## 1) 项目定位（1 段话）
NetOS 是一个以“本地优先 + 发现式同步”为核心的分布式缓存同步系统原型，目标是在非 RDMA 低成本网络上接近 RDMA 访问延迟。当前仓库是 **Phase‑1 Demo**：Redis + 同步进程同容器，UDP REQ/DATA 拉取路径，强调可运行、可演示、可迭代。
...
- ⛔️ **尚未实现**：Push、Bloom Filter、异步转发、动态拓扑。
...
- `src/netos/tables/`：QueryTable / SyncTable
